...
import {add_sell,get_one_sell, update_sell} from "@/app/utils/network/sell" 
...
const SellAddAdmin = (props)=>{
    ...
    if(props.data_set != null){
        if(!is_get){
            get_one_sell(props.data_set).then((data)=>{
                set_name(data.name)
    ...
    return (
        <div className="w-full flex flex-col items-start gap-y-[20px] overflow-y-scroll p-[30px]">
            ...
            <input value={price} onChange={(e)=>{set_price(e.currentTarget.value)}} className="bg-[rgba(0,0,0,0.1)] rounded p-[5px] md:w-[300px] w-full" placeholder="قیمت..." />
            ...
            <button onClick={()=>{
                if(props.data_set == null){
                    add_sell(name,image_list,labels.split("|"),features,meterage,price,address).then((data)=>{
                        if(data.status == 200){
                            props.set_select_page(4)
                        }else{
                            set_status("اطلاعات را کامل وارد نماییید")
                        }
                    })
                }else{
                    update_sell(props.data_set,name,image_list,labels.split("|"),features,meterage,price,address).then((data)=>{
                        if(data.status == 200){
                            props.set_select_page(4)
                        }else{
                            set_status("اطلاعات را کامل وارد نماییید")
